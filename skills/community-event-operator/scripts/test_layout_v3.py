...
font_bottom_heavy = ImageFont.truetype(FONTS[lang_key]["title"], 34) # Bolder and bigger
text_color = (0, 0, 0)
...
paste_y = 170 # Moved up from 230
img.paste(txt_rotated, (40, paste_y), mask=txt_rotated)

# --- B. 上方左侧：两行核心亮点 (Bottom Left, Bigger & Bolder) ---
y_left_info = 1260 # moved slightly up to accommodate bigger font
for h in highlights[:2]: 
    draw.text((100, y_left_info), f"• {h}", font=font_bottom_heavy, fill=text_color)
    y_left_info += 55
...
# --- E. 右下角细节区 (Right Aligned, Bigger & Bolder) ---
info_y = 1260 # moved slightly up
time_str = f"时间: {event_data.get('time', 'TBD')}"
loc_str = f"地点: {event_data.get('location', 'TBD')}"
fee_str = f"费用: {event_data.get('fee', 'Free')}"

right_align_text(draw, 960, info_y, time_str, font_bottom_heavy, text_color)
right_align_text(draw, 960, info_y + 50, loc_str, font_bottom_heavy, text_color)
right_align_text(draw, 960, info_y + 100, fee_str, font_bottom_heavy, text_color)

img.save(os.path.join(BASE_DIR, "../outputs/preview_layout_v3.png"))
print("Saved preview_layout_v3.png")
